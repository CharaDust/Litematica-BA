"""与 UI 测试页 ``ContentListTableWidget`` 对齐的纯文本 ``QTableWidget`` 主题样式。

Metro8 / Metro10：透明表头、无网格、行悬停/选中底色与内容列表一致。
其它主题：使用 Qt 默认表格绘制，但行高与内容列表一致——除 QTDefault 外均为 60px（含 Minecraft）。
"""
...
from litematicaba.ui.content_display.list_table.view import (
    METRO_LIST_SAMPLE_ROW_HEIGHT,
    METRO_LIST_TABLE_CHROME_QSS,
    is_metro_list_table_theme,
    uses_content_list_sample_row_height,
)
...
class _ViewportHoverFilter(QObject):
    ...
    def eventFilter(self, obj: QObject, event: QEvent) -> bool:  # type: ignore[override]
        if obj is self._table.viewport() and is_metro_list_table_theme(self._table.theme_id):
...
class ThemedPlainQTableWidget(QTableWidget):
    ...
    def sync_row_heights(self) -> None:
        """在数据行变更或主题切换后调用，使行高与 UI 测试页内容列表一致（见 ``uses_content_list_sample_row_height``）。"""
        if uses_content_list_sample_row_height(self._theme_id):
            for r in range(self.rowCount()):
                self.setRowHeight(r, METRO_LIST_SAMPLE_ROW_HEIGHT)
        else:
            for r in range(self.rowCount()):
